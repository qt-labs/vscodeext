...
func validateFileName(fl validator.FieldLevel) bool {
	s := fl.Field().String()
	return utils.IsValidFileName(s)
}

func validateDirName(fl validator.FieldLevel) bool {
	s := fl.Field().String()
	return utils.IsValidDirName(s)
}
...
func validateWindowsDrive(fl validator.FieldLevel) bool {
	...
	if runtime.GOOS == "windows" {
		return utils.HasValidWindowsDrive(s)
	}
...
func validateSafeProjectName(fl validator.FieldLevel) bool {
	...
	if runtime.GOOS == "windows" {
		return !utils.IsWindowsReservedName(fl.Field().String())
	}
...
var TagToValidatorMessage = map[string]string{
	TagRequired:        texts.ValidatorTagRequired,
	TagMinLength:       texts.ValidatorTagMinLength,
	TagMaxLength:       texts.ValidatorTagMaxLength,
	TagMatch:           texts.ValidatorTagPattern,
	TagDirName:         texts.ValidatorTagDirName,
	TagFileName:        texts.ValidatorTagFileName,
	TagAbsPath:         texts.ValidatorTagAbsPath,
	TagSafeFileName:    texts.ValidatorTagSafeFileName,
	TagSafeProjectName: texts.ValidatorTagSafeProjectName,
	TagWindowsDrive:    texts.ValidatorTagWindowsDrive,
}
