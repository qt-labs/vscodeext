...
func (f *UserPresetFile) Open() error {
	...
	if !utils.EntryExists(f.filePath) {
...
func (f *UserPresetFile) FindByUniqueId(id string) (PresetData, error) {
	...
	return PresetData{},
		fmt.Errorf(utils.Msg("not found, given = '%v'"), id)
}
...
func (f *UserPresetFile) FindByName(name string) (PresetData, error) {
	...
	return PresetData{},
		fmt.Errorf(utils.Msg("not found, given = '%v'"), name)
}
...
func (f *UserPresetFile) Save() error {
	...
	_, err = utils.WriteAll([]byte(output), f.filePath)
	if err != nil {
		return err
	}
...
func (f *UserPresetFile) Remove(name string) error {
	...
	if found < 0 {
		return fmt.Errorf(utils.Msg("not found, given = '%v'"), name)
	}
...
func (f *UserPresetFile) Rename(from string, to string) error {
	...
	if err == nil {
		return fmt.Errorf(
			utils.Msg("cannot rename, already exist, given = '%v'"), to)
	}
